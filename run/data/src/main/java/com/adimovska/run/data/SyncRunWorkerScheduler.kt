...
import com.adimovska.core.database.mappers.toRunEntity
...
class SyncRunWorkerScheduler(
    ...
    private suspend fun scheduleFetchRunsWorker(interval: Duration) {
        ...
        val workRequest = PeriodicWorkRequestBuilder<FetchRunsWorker>(
            ...
            .setBackoffCriteria( //if our worker fails, how long do we want to wait till the next retry
                backoffPolicy = BackoffPolicy.EXPONENTIAL, //exponential means that the first retry will be after 2s
                backoffDelay = 2000L,  // the next one after 4, then 8 etc etc
